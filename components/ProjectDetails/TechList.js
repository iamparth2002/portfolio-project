    ...
    <div className="mt-10 p-5">
      ...
      <div className="grid grid-cols-3 text-center gap-2 mt-2">
        {project?.techList.map((tech, index) => (
          <h2
            key={index}
            className="border-[1px] border-sky-300
           px- rounded-full text-[14px]"
          >
            {tech}
          </h2>
        ))}
      </div>
      <div className="w-[350px] text-wrap">
        <h2 className="font-bold mt-6">Source/Links</h2>
        {project['app-demo-url'] ? (
          <h2 className="font-light mt-4 cursor-pointer flex items-center gap-2 text-wrap">
            <Image
              src="/Images/play.png"
              width={20}
              height={20}
        ...
        {project['yt-url'] ? (
          <h2
            className="font-light mt-4 cursor-pointer 
            flex items-center gap-2 text-wrap"
            onClick={() => window.open(project['yt-url'])}
          >
            <Image
        ...
        {project['ui-ux-design-url'] ? (
          <h2
            className="font-light max-w-[75ch]
            flex items-center gap-2 mt-4 cursor-pointer"
            onClick={() => window.open(project['ui-ux-design-url'])}
          >
            <Image
